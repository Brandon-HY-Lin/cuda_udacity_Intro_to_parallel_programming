...
template <typename T>
void checkResultsEps (const T* const ref, const T* const gpu, std::size_t numElem, double eps1, double eps2)
{
	assert(eps1 >= 0 && eps2 >= 0);
	unsigned long long totalDiff = 0;
	unsigned numSmallDifferences = 0;
	for (std::size_t i = 0; i < numElem; ++i) {
	// subtract smaller ffrom larger in case of unsigned types
