...
void maskPredicateKernel(
	...
	// on the border, or if its on the interiror border
	...
	if (masked(d_sourceImg[m])) {
		...
		if (withinBounds(p.x, p.y + 1, numRowsSource, numColsSource)) {
			...
			// Is lower pixel inside the mask?
		...
		if (withinBounds(p.x, p.y - 1, numRowsSource, numColsSource)) {
			...
			// Is upper pixel inside the mask?
			if (masked(d_sourceImg[getm(p.x, p.y - 1, numColsSource)]))
				interior++;
		...
		if (withinBounds(p.x + 1, p.y, numRowsSource, numColsSource)) {
			...
			// Is right pixel inside the mask?
...
__global__
void jacobiKernel(
	float* d_in,	// input (initial temporary foreground)
	float* d_out,	// output
	const int* d_borderPredicate,
	const int* d_interiorPredicate,
	float* d_source,	// foreground image
	float* d_dest,		// background image
	size_t numRows,
	size_t numCols)
{
	// step 1: copy forground if adjacent index is in interior pixels list
	// step 2: copy background if adjacent index is in the border pixels list
	// step 3: average the foreground pixels, backgrond pixels and current pixel.
	const int2 p = get2dPos();
	const int  m = getm(p.x, p.y, numCols);
	...
	// if center pixels is inside interior.
	if (d_interiorPredicate[m] == 1) {
		float a = 0.f, b = 0.f, c = 0.0f, d = 0.f;
		float sourceVal = d_source[m];

		// if lower index is valid.
		if (withinBounds(p.x, p.y + 1, numRows, numCols)) {
			d++; // increment number of pixels

			// get index of lower pixel
			lm = getm(p.x, p.y + 1, numCols);
			if (d_interiorPredicate[lm] == 1) {
				a += d_in[lm]; // copy temparary foreground
			}
			else if (d_borderPredicate[lm] == 1) {
				b += d_dest[lm]; // copy background
			}
			c += (sourceVal - d_source[lm]);   // compute g value (gradient?)
		}
		// if upper index is valid
		if (withinBounds(p.x, p.y - 1, numRows, numCols)) {
			d++; // increment number of pixels
			// get index of upper pixel
			lm = getm(p.x, p.y - 1, numCols);
			if (d_interiorPredicate[lm] == 1) {
				a += d_in[lm]; // copy temp foreground 
			}
			else if (d_borderPredicate[lm] == 1) {
				b += d_dest[lm]; // copy background
			}
			c += (sourceVal - d_source[lm]); // compute gradient ?
		}

		// if index of right pixel is valid
		if (withinBounds(p.x + 1, p.y, numRows, numCols)) {
			d++; // increment number of pixels
			// get index of right pixel
		...
		// if index of left pixel is valid
		if (withinBounds(p.x - 1, p.y, numRows, numCols)) {
			d++; // increment number of pixels
			// get index of left pixel
			lm = getm(p.x - 1, p.y, numCols);
			if (d_interiorPredicate[lm] == 1) {
				a += d_in[lm]; // copy temp foreground
			}
...
void blend_gpu(const uchar4* const h_sourceImg, // IN  (foreground image)
	const size_t numRowsSource, const size_t numColsSource,
	const uchar4* const h_destImg, // IN (background image)
	uchar4* const h_blendedImg) // OUT (composite image)
{
	...
	const dim3 gridSize((numColsSource - 1) / blockSize.x + 1, (numRowsSource - 1) / blockSize.y + 1);
	...
	**/
	...
	for (int i = 0; i < 800; i++) {
		...
		jacobiKernel << <gridSize, blockSize >> > (
			...
			d_borderPredicate,
			d_interiorPredicate,
			d_sourceImgR, // foreground image (red)
			d_destImgR, // background image (red)
			numRowsSource,
			numColsSource
			);
		std::swap(d_r0, d_r1);
		...
		jacobiKernel << <gridSize, blockSize >> > (
			...
			d_borderPredicate,
			d_interiorPredicate,
			d_sourceImgG, // foreground image (green)
			d_destImgG, // background image (green)
			numRowsSource,
			numColsSource
			);
		std::swap(d_g0, d_g1);
		...
		jacobiKernel << <gridSize, blockSize >> > (
			...
			d_borderPredicate,
			d_interiorPredicate,
			d_sourceImgB,	//foreground image (blue)
			d_destImgB,		// background image (blue)
			numRowsSource,
			numColsSource
			);
		std::swap(d_b0, d_b1);
