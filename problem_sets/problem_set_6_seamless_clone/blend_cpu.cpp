...
void computeIteration( const unsigned char* const dstImg,
						...
						const std::size_t numColsSource,
						const float* const f,
						const float* const g,
						float* const f_next)
{
	...
	for (std::size_t i = 0; i < interiorPixelList.size(); ++i) {
		...
		if (strictInteriorPixels[offset - 1]) {
			/*   asterisk (*) means strict interior, hypen (-) means don't care.
			 *
			 *        -
			 *      * o -
			 *        -
		...
		/* asterisk (*) means interior, hashtag (#) means border, hypen (-) means don't care.
		*
		*  f_next_val = blendedSum   +    borderSum      +   g[offset]
		*               (f_cur_val)      (const image)      (const gradinet?)
		*                                (pool)             (gradient of bear?)   
		*
		*
		* blendedSum = 
		*
		...
		* borderSum = 
		* 
		*   dstImg[-1, 0] + dstImg[1, 0]  + dstImg[0, -1] + dstImg[0, 1]
		*
		*        -              -               #                -
		*      # o -      +   - o #       +   - o -       +    - o -
		*        -              -               -                #
				...
				const std::vector<uint2>& interiorPixelList)
{
	for (std::size_t i = 0; i < interiorPixelList.size(); ++i) {
		...
		/*  all adjacent pixels belong to mask.
		 *
		 *        -1
		 *    -1   4  -1
		 *        -1
...
void blend_cpu (const uchar4* const h_sourceImg,
				const std::size_t numRowsSource, const std::size_t numColsSource,
				const uchar4* const h_destImg,
				uchar4* const h_blendedImg)
{
	// we need to create a list of border pixels and interior pixels
	// this is a conceptually simple implementation, not a particcularly efficient one...

	// first create mask
	std::size_t srcSize = numRowsSource * numColsSource;
	...
	for (std::size_t r = 1; r < numRowsSource - 1; ++r) {
		for (std::size_t c = 1; c < numColsSource - 1; ++c) {
			if ( mask[r *numColsSource + c]) {
				...
				if (mask[(r - 1) * numColsSource + c] && mask[(r+1) * numColsSource + c] &&
					...
					*/   
					...
					/* Not all adjacnt pixels belong to mask.
					 *
					 *        0
					 *      0 1 1
					 *        1
				...
				/* itself does not belong to mask.
				 *
				 *        -
				 *      - 0 -
				 *        -
	...
	const std::size_t numIterations = 800;
