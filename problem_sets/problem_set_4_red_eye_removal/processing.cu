...
/*
 * - read image and template image
 * - transform array of structures (AoS) into structure of arrays (SoA)
 * - normalize each color channel
 * - calculate correlation between image and template image
 * - 
 */
void preProcess(unsigned int **inputVals,
  ...
  checkCudaErrors(cudaFree(0));
	...
	// split the image, and store results into (d_red, d_green, d_blue)
	//	d_red[0], d_green[0], d_blue[0] = d_Img[0].x, d_Img[0].y, d_Img[0].z
	...
	// combine data: d_combined_response = (d_red, d_blue, d_green)
	...
	// allocate device mem with value = [0..numElem]
	thrust::device_vector<unsigned int> coords(numElem);
	thrust::sequence(coords.begin(), coords.end()); // coords=[0..numElem -1]
